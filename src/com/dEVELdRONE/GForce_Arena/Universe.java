...
public class Universe extends SurfaceView implements Runnable{
	...
	public void addParticles(int h, float xmin, float ymin, float xmax, float ymax){
		for(int a=0; a<h; a++) //btw(xmin+1 to xmax-1)
			addParticle((int)(Math.random()*(xmax-xmin-2)+1+xmin), (int)(Math.random()*(ymax-ymin-2)+1+ymin));
	}
