 ...
 * 
 * v8
 * Some files for levels are included. We're still working on creating levels, but don't worry. They'll be here soon!
 * Some new functionalities for multitouch added
 * 
 */
...
public class ArcadeActivity extends Activity implements OnTouchListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
      ...
      //enables full screen with no title
        ...
        sound = new BackgroundSound(this, R.raw.background);
	...
	public boolean onTouch(View v, MotionEvent event) {
		if(multitouch){
			...
			if(pointerCount<5){
				Particle.setIndiv(pointerCount>2);
				Particle.setLoseM(!(pointerCount==2));
			}
	...
	protected void onResume() {
		...
		boolean soundIsOn = getPrefs.getBoolean("music", true);
		sound.setIsOn(soundIsOn);
		if(soundIsOn) 
			sound.play();
		else sound.pause();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_rate_it:
			...
			//intent.setData(Uri.parse("http://www.amazon.com/dEVELdRONE-G-Force-Arena/dp/B006Z0XRYA/"));
